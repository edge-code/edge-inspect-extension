...
define(function (require, exports, module) {
    ...
    function followOrRemoteInspect(event, url) {
        if (SkyLabView.shouldFollowThisUrl(url)) {
            SkyLabController.followUrl('http://' + hostip + ':' + port + '/' + url, [], "false");
        }
    }
...
//        SkyLabController.setUrl('http://' + hostip + ':' + port + '/' + docurl);
...
//        SkyLabView._toasthandle.show();
    ...
    SkyLabView.triggerFollowModeOn = function () {
        var hosts = SkyLabController.getDeviceManagerInfo();
        hostip = hosts.hostips[0];
        attachTabWatchers();
