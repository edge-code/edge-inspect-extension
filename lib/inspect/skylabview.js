...
define(function (require, exports, module) {
    ...
    function followOrRemoteInspect(url) {
        if (SkyLabView.shouldFollowThisUrl(url)) {
            SkyLabController.followUrl(url, [], "false");
        }
    }
    // Function to poll the URL bar for changes.
    function watchForURLChange() {
        EventMap.subscribe('Inspect:urlchange', function (evt) {
            followOrRemoteInspect(evt.inspectURL);
        });
    }
    ...
    function initializeExtension() {
        ...
        SkyLabController.setUrl('http://127.0.0.1:8007/');
